...
//const double constant = 1.0; // test to see what happens (is currently used in the current version of oomph-lib)
...
public:
    vector<Node*> nodes; // (3)
    MeshElementFace(vector<Node*> nodes);
    double area();
    vector<double> face_vector(); // (3)
};

MeshElementFace::MeshElementFace(vector<Node*> nodes)
{
    this->nodes = nodes;
}

double MeshElementFace::area()
{
    // defining vectors
    vector<double> v1(3);
    vector<double> v2(3);
    ...
    {
        v1[i] = nodes[1]->coord[i] - nodes[0]->coord[i];
        v2[i] = nodes[2]->coord[i] - nodes[0]->coord[i];
    }

    //calculates area
    double area2 = 0;
    for (unsigned k=0; k<3; k++) // iterate of each element of the area vector
    {
        double element = 0;
        // iterate over the a_i and b_j to find area element k
        for (unsigned i=0; i<3; i++)
        {
            for (unsigned j=0; j<3; j++)
            {
                element += levi_civita(i, j, k) * v1[i] * v2[j];
            }
        }
        area2 += pow(element, 2);
    }

    return 0.5 * sqrt(area2);
}

// cross product between two edge vectors in the local coordinates
vector<double> MeshElementFace::face_vector()
{
    // defining vectors
    vector<double> v1(3);
    vector<double> v2(3);
    ...
    // take cross product
    vector<double> vc(3); // [k] = \epsilon_ijk a_i b_j

    for (unsigned k=0; k<3; k++) // loop over each term of the resultant vector
    {
        double k_value = 0;
        for (unsigned i=0; i<3; i++) // loop over a and b (calculate the vector element k)
        {
            for (unsigned j=0; j<3; j++)
            {
                k_value += levi_civita(i, j, k) * v1[i] * v2[j];
            }
        }
        vc[k] = k_value;
    }
...
// (in radians)
double angle_between_faces(MeshElementFace* f1, MeshElementFace* f2)
{
    vector<double> a = f1->face_vector();
    vector<double> b = f2->face_vector();

    double cos_theta = dot_product(a,b) / (magnitude(a) * magnitude(b));
...
public:
    ...
    vector<MeshElementFace> get_faces();
    double quality(); // quality measure (\gamma) as defined in section 4.1 (https://gmsh.info/doc/preprints/gmsh_paper_preprint.pdf)

    double max_edge_length();

private:
    double get_volume();
//    std::pair<Node*, Node*> get_max_length_edge();
    double length_of_edge(pair<Node*, Node*> edge);
};
...
vector<MeshElementFace> Element::get_faces()
{
    return {
        MeshElementFace({&nodes[0], &nodes[1], &nodes[2]}),
        MeshElementFace({&nodes[0], &nodes[1], &nodes[3]}),
        MeshElementFace({&nodes[0], &nodes[2], &nodes[3]}),
        MeshElementFace({&nodes[1], &nodes[2], &nodes[3]}),
    };
}

double Element::length_of_edge(pair<Node*, Node*> edge)
{
    double r2 = 0;
    for (unsigned i=0; i<edge.first->coord.size(); i++)
    {
        r2 += pow(edge.first->coord[i] - edge.second->coord[i], 2);
    }
    return sqrt(r2);
}

double Element::max_edge_length()
{
    vector<pair<Node*, Node*>> edges = get_edges();
    double max_length = 0;
    for (unsigned i=0; i<edges.size(); i++)
    {
        if (length_of_edge(edges[i]) > max_length) max_length = length_of_edge(edges[i]);
    }
    return max_length;
}
...
double Element::quality()
{
    double v = volume;
    double max_edge_length = this->max_edge_length();
    vector<MeshElementFace> faces = this->get_faces();

    double sum_of_face_areas = 0;
    for (unsigned i=0; i<faces.size(); i++)
    {
        sum_of_face_areas += faces[i].area();
    }

    double num = 6 * sqrt(6) * volume;
    double den = sum_of_face_areas * max_edge_length;
...
//    gmsh::view::write(view_tag, "example_refine.pos");
...
void basic_experiment()
{
    ...
    int f_tag = add_uniform_target_size_field(target_size);
    ...
    for (unsigned i=0; i<8*6+2; i++)
    {
        cout << " ";
    }
    cout << "average   quality   max l" << endl;
...
//    double counter = 0;
    for (unsigned i=0; i<element_pts.size(); i++)
    {
        ...
        // calculate length of tet element edges
        ...
        // get quality measure \gamma
//        element_pts[i]->

        double average = total / double(edges.size());
        printf("- %.5lf - %.5lf - %.5lf - %.5lf\n", average, element_pts[i]->quality(), element_pts[i]->max_edge_length(), element_pts[i]->volume);
//        cout << "- " << average << " - " << element_pts[i]->quality() << " " << element_pts[i]->max_edge_length() << endl;/**/

        fstream f;
        f.open("data.txt", ios_base::app);
        f << average << " " << element_pts[i]->quality() << " " << element_pts[i]->midpoint_coords[0]
        << " " << element_pts[i]->midpoint_coords[1] << " " << element_pts[i]->midpoint_coords[2] << '\n';
        f.close();
...
void remove_min_max_experiment()
{
    gmsh::initialize();
    gmsh::clear();
    ...
    const vector<pair<int, int>> dim_tags = {pair<int, int>(3,1)};
    vector<pair<int, int>> out_dim_tags;
    gmsh::model::getBoundary(dim_tags, out_dim_tags);
    for (unsigned i=0; i<out_dim_tags.size(); i++)
    {
        cout << out_dim_tags[i].first << ", " << out_dim_tags[i].second << endl;

        int tag = out_dim_tags[i].second;
        string data_type;
        vector<size_t> tags;
        vector<vector<double>> data;
        double time;
        int num_components;
        gmsh::view::getModelData(0, 0, data_type, tags, data, time, num_components);

        cout << "data_type: " << data_type << ", tags length: " << tags.size() << endl;
    }

    cout << "out_dim_tags Length: " << out_dim_tags.size() << endl;
    ...
    cout << "average   new-avg   quality   max l" << endl;
...
//    double counter = 0;
    for (unsigned i=0; i<element_pts.size(); i++)
    {
        ...
        vector<double> edge_lengths(edges.size());
        ...
        // average element edge length excluding the
        double min_length = edge_lengths[0];
        double max_length = edge_lengths[0];

        for (unsigned j=1; j<edge_lengths.size(); j++)
        {
            if (edge_lengths[j] < min_length) min_length = edge_lengths[j];
            if (edge_lengths[j] > max_length) max_length = edge_lengths[j];
        }

        double new_total = total;
        double new_counter = double(counter);

        new_counter -= 2;
        new_total -= min_length;
        new_total -= max_length;

        double new_average = new_total / double(new_counter);

        double average = total / double(edges.size());
        printf("- %.5lf - %.5lf - %.5lf - %.5lf - %.5lf\n", average, new_average, element_pts[i]->quality(), element_pts[i]->max_edge_length(), size_from_volume(element_pts[i]->volume));

        fstream f;
        f.open("data_selective_average.txt", ios_base::app);
        f << average << " " << new_average << " " << element_pts[i]->quality() << " " << element_pts[i]->midpoint_coords[0]
          << " " << element_pts[i]->midpoint_coords[1] << " " << element_pts[i]->midpoint_coords[2] << '\n';
        f.flush();
        f.close();


        // angle between element faces
        vector<MeshElementFace> faces = element_pts[i]->get_faces();
        vector<double> dihedral_angles;
        double dihedral_angle_total = 0;
        double dihedral_angle_counter = 0;
        for (unsigned k=0; k<3; k++)
        {
            for (unsigned l=k+1; l<4; l++)
            {
                // TODO: finish angle between each mesh element face pairs
                double angle = angle_between_faces(&faces[k], &faces[l]);
                cout << angle * 180.0 / 3.14159265  << " ";

                dihedral_angles.push_back(angle);
                dihedral_angle_total += angle;
                dihedral_angle_counter += 1;
            }
        }

        double dihedral_angle_average = dihedral_angle_total / dihedral_angle_counter;

        // variance of dihedral angles
        double dihedral_angle_variance = 0;
        for (unsigned j=0; j<dihedral_angles.size(); j++)
        {
            dihedral_angle_variance += (dihedral_angles[j] - dihedral_angle_average) * (dihedral_angles[j] - dihedral_angle_average) / (dihedral_angles.size() - 1);
        }

        cout << endl << dihedral_angle_variance;
        cout << endl;

//        fstream f;
        f.open("data_dihedral_angle_variance_volume.txt", ios_base::app);
        f << element_pts[i]->volume << "," << dihedral_angle_variance << '\n';
        f.flush();
        f.close();
...
//    MeshElementFace face = MeshElementFace({});
//    cout << "area: " << face.area() << endl;

    remove_min_max_experiment();
...
//        if (iter%8 == 0)
        if (true)
        {
            string filename = "../../out/bg_msh/grid_stability_3_cube_its=";
            filename.append(to_string(target_size));
            filename.append("_c=");
            filename.append(to_string(constant));
            filename.append("_N=");
