...
//            tet_target_sizes[i] = e.linear_target_size;
            if (e_i == 4) {
                tet_target_sizes[i] = 1.5;
                cout << "*********" << tet_target_sizes[i] << "*********" << endl;
            } else {
                tet_target_sizes[i] = 3.;
            }
        }
    ...
    gmsh::view::write(view_tag, "target_size.pos");
...
void analyse_mesh_element_volumes()
{
//    cout << "grid based refinement using bg msh, target_size: " << target_size <<  " C = " << constant << ", N = " << N << endl;
...
//    add_bar_bend_twist_t3(); // For different test shapes, change what function is called here
    ...
    string filename = "element_vols.txt";
    fstream f;
    f.open(filename, ios_base::app);
...
//    add_bar_bend_twist_t3(); // For different test shapes, change what function is called here
    ...
    // Extract mesh data
    vector<Element> uniform_elements = get_elements();

    cout << "**********************" << endl;
    cout << "**********************" << endl;
    for (unsigned i=0; i<uniform_elements.size(); i++)
    {
//        uniform_elements[i].calculate_midpoint();
//        cout << i << " " << to_string(uniform_elements[i].midpoint_coords) << endl;

        for (unsigned j=0; j<3; j++)
        {
            if ( (uniform_elements[i].nodes[j].coord[0] == 0) && (uniform_elements[i].nodes[j].coord[1] == 0) && (uniform_elements[i].nodes[j].coord[2] == 0) )
            {
                cout << i << " " << to_string(uniform_elements[i].nodes[0].coord) << " " << to_string(uniform_elements[i].nodes[1].coord) << " " << to_string(uniform_elements[i].nodes[2].coord) << " " << to_string(uniform_elements[i].nodes[3].coord) << endl;
            }
        }
    }
    cout << "**********************" << endl;
    ...
    // save uniform mesh
    gmsh::write("uniform.msh");

    // Once mesh data has been extracted and saved, GMSH is finished with for this iteration.
    gmsh::clear();
    gmsh::finalize();
    ...
    int refined_target_size_field = add_refined_mesh_to_gmsh(uniform_elements);
    mesh_with_bg_fields({double(refined_target_size_field)});

    // save refined mesh
    gmsh::write("refined.msh");

    // Extract refined mesh data
    vector<Element> refined_elements = get_elements();

    // Once mesh data has been extracted and saved, GMSH is finished with for this iteration.
    gmsh::clear();
    gmsh::finalize();

    cout << "uniform_elements.size() = " << uniform_elements.size() << endl;
    cout << "refined_elements.size() = " << refined_elements.size() << endl;
}

int main()
{
    refine_single_element();
    return 0;


    double target_size = 1;
    double target_size_increment = 1;
    for (unsigned i=0; i<10; i++)
    {
        target_size += target_size_increment;
