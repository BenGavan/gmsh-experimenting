...
int main()
{
    ...
    for (unsigned i=0; i<10; i++) {
        ...
        auto meshSizeCallback = [=](int dim, int tag, double x, double y, double z) {
            if (x < 0.5 && y < 0.5 && z < 0.5) return size; // inside corner cube
            return 1.;
        };
        ...
        string filename = "mesh_";
        filename.append(to_string(i));
        filename.append(".msh");
        gmsh::write(filename);

        // count tetrahedron elements
        int num_elements = getNumberOfTetrahedronMeshElementTags();
