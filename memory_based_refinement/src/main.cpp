...
/// Target Size Field ///
class TargetSizeField{
public:
    virtual double target_size_for_coord(vector<double> x);
    virtual ~TargetSizeField() {};
};

/// Target Size Box : Target Size Field ///
class TargetSizeBox : public TargetSizeField {
public:
    double target_size_for_coord(vector<double> x) override;

    double v_in, v_out;
    vector<double> min_vertex, max_vertex;

    TargetSizeBox(double v_in, double v_out, vector<double> min_vertex, vector<double> max_vertex) : v_in(v_in),
                                                                                                     v_out(v_out), min_vertex(min_vertex), max_vertex(max_vertex) {}
};

double TargetSizeBox::target_size_for_coord(vector<double> x)
{
    if (x[0] > min_vertex[0] && x[1] > min_vertex[1] && x[2] > min_vertex[2] &&
        x[0] < max_vertex[0] && x[1] < max_vertex[1] && x[2] < max_vertex[2]) {
        return v_in;
    }
    return v_out;
}
...
/// Target Size MinMax  : Target Size Field ///
class TargetSizeMinMax : public TargetSizeField {
public:
    TargetSizeMinMax(vector<TargetSizeField*> fields);
    vector<TargetSizeField*> fields;
    bool is_min = true;  // if is_min == false, take the max field, if is_min == true, take the min of the fields
};
...
void next()
{
    ...
    TargetSizeBox target_box(0.2, 1, {1, 1, 1}, {2, 2, 2});

    fields.push_back(&target_box);

    TargetSizeMinMax min_max_field(fields);

    for (unsigned i=0; i<min_max_field.fields.size();  i++)
    {
        cout << min_max_field.fields[i]->target_size_for_coord({0, 0, 0}) << endl;
    }
}
